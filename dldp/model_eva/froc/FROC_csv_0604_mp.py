...
------------
    ...
    Input:  heatmap (two heatmaps, one is for generating binary image;
            ...
            dimension file
            Please pay attention to the index number of three lists: label_heatmap_list, value_heatmap_list, dim_path_list.
    Output: CSV and processed heatmap files for each slide
...
Please move test_114.csv to a folder since test_114 is removed from FROC test since its annotations on tumor region is not exhaustive.

If the resolution of a heatmap needs to be reduced, the position ("prod_coord")calculation method also needs to be changed.

If slide level detection scores are used, the reference file and the related column name needs to be provided.
        slide_level_pred = ROC_result['PLEASE INPUT THE RELATED COLUMN NAME'].at[i]
...
def lesion_coord(i):
    ...
    for index in range(number_lesion):
        ...
        prob_coord = [probability_ave, y * 16 * 4*2 + dim_x*224,
                      x * 16 * 4*2 + dim_y*224]  # 10-29-2019: for reduced map
...
def lesion_coord_with_roc(i, based_on_roc=True):
    ...
    value_heatmap = np.load(value_heatmap_path)
    ##################################################################################################################
    #label_heatmap = label_heatmap[0::8, 0::8]
    #value_heatmap = value_heatmap[0::8, 0::8]
    label_heatmap = label_heatmap[7::14, 7::14]
    value_heatmap = value_heatmap[7::14, 7::14]
    ##################################################################################################################
...
#       row_index = int(ref[ref['slide_name'].str.find(case[0:-4])!=-1].index[0])
        ###############################################################################################################
        #slide_level_pred = ROC_result['scores_method_II_64_stride_color_norm_9.0'].at[i]
        slide_level_pred = ROC_result['scores_method_II_224_stride_color_norm_8.0'].at[i]
        ###############################################################################################################
        ...
        if slide_level_pred > 0.5:
            ...
            for index in range(number_lesion):
                ...
                #probability_ave = probability_ave
                probability_ave = probability_ave*slide_level_pred
                ################################################################################################################
                #prob_coord = [probability_ave, y * 16 + dim_x*224, x * 16 + dim_y*224]
                prob_coord = [probability_ave, y * 224 + dim_x*224, x * 224 + dim_y*224] # for 224 heatmap downsample
                #prob_coord = [probability_ave, y * 16 * 4*2 + dim_x*224,
                #                x * 16 * 4*2 + dim_y*224]  # 07-19-2020: for reduced map
                ################################################################################################################
                #prob_coord = [probability_ave, y * 16 + dim_x*224, x * 16 + dim_y*224]
                #prob_coord = [probability_ave, x * 16 + dim_x*224, y * 16 + dim_y*224]
                #prob_coord = [probability_ave, y * 16 + dim_x, x * 16 + dim_y]
                print(prob_coord)
                #if region_props[index]['area'] > 80:
                froc_csv.append(prob_coord)
    froc_csv_data = pd.DataFrame(froc_csv, columns=col)
    # Save to CSV file with the name same as its slide name.
...
if __name__ == '__main__':
    ...
    value_heatmap_paths = list_heatmap_file_in_dir(value_heatmap_path_list[2])
    value_heatmap_paths.sort()
    print('number of value_heatmap: %d' % len(value_heatmap_paths))

    label_heatmap_paths = list_heatmap_file_in_dir(label_heatmap_path_list[2])
    ...
    dim_paths = glob.glob(osp.join(dim_path_list[2], '*.npy'))
    dim_paths.sort()
    print(dim_paths)
    ...
    if based_on_roc:
        ROC_result = pd.read_csv(
            #'/scratch/weizhe.li/RF_trained_models/RF_models_color_norm_repeat/stride_64/9.0/reference_with_redo_results_16_stride_9.0.csv')
            '/scratch/weizhe.li/RF_trained_models/RF_models_color_norm_repeat/stride_224/8.0/reference_with_redo_results_224_stride_8.0.csv')
    #########################################################################################################################################    
    result_folder = '/projects01/wxc4/wli/CAMELYON16/FROC/test_kernel_1_0724_stride_224'
    try:
    ...
    for i in range(len(label_heatmap_paths)):
        p = Process(target=lesion_coord_with_roc, args=(i,))
        p.start()

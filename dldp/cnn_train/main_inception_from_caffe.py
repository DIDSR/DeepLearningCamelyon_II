...
def step_decay(epoch):
    ...
    epochs_drop = 1.0
    lrate = initial_lrate * \
        math.pow(drop, math.floor((epoch+1) / epochs_drop))
...
def model_train(train_generator, validation_generator, model_name_path,
    ...
    model_checkpoint = ModelCheckpoint(
        model_name_path, monitor='val_prob_accuracy', verbose=1, save_best_only=True)
    ######################################################################
    ...
    model.save(
        '/%s/model_%s_%s-{epoch:02d}-{val_prob_accuracy:.4f}.h5' % (
            sub_folder, current_time, patches_category))

    model_json = model.to_json()
    with open("/%s/%s_%s_%s-{epoch:02d}-{val_prob_accuracy:.4f}.json" % (
            sub_folder, model_name, current_time, patches_category), "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights(
        "/%s/%s_%s_%s-{epoch:02d}-{val_prob_accuracy:.4f}.h5" % (
...
if __name__ == "__main__":
    ...
    image_patch_dirs = ['/home-ORG/weizhe.li/Documents/original_256_patches']
    ...
    path_to_save_model = '/home-ORG/weizhe.li/cnn_train_0914'
    model_name = 'no_noise_no_norm_090220_with_preprocess_with_decay'
    ...
    for index, image_patch_dir in enumerate(image_patch_dirs):
        ...
        for training_type in training_types:
            ...
            # os.mkdir(sub_folder)
            model_name_path = '/%s/googlenetv1_caffe_no_decay_%s_%s_%s-{epoch:02d}-{val_prob_accuracy:.4f}.hdf5' % (
                sub_folder, model_name, current_time,
                patches_category)
            if training_type == 'no_color_noise':
                train_generator = tpp.gen_imgs_caffe(
                    training_patches, BATCH_SIZE, crop_size, folder_to_save)
                validation_generator = tpp.gen_imgs_caffe(
                    validation_patches, BATCH_SIZE, crop_size, folder_to_save)
            ###################################################################
            # train the neural network
            ########
                history = model_train_caffe(train_generator, validation_generator,
                                      model_name_path,
                                      model_name, sub_folder,
...
#
            ...
            plt.plot(history.history['prob_loss'])
            plt.plot(history.history['val_prob_loss'])

...
"""
# new keras does not have the following:
...
from keras.initializers import he_normal  # Or your initializer of choice
import warnings
...
def conv2d_bn(x,
    ...
    x = Conv2D(
        ...
        use_bias=False, name=conv_name, kernel_initializer='glorot_uniform', kernel_regularizer=l2(0.0005))(x)
    if normalizer:
...
def concatenated_block(x, specs, channel_axis, name):
    ...
    branch_3 = MaxPooling2D((3, 3), strides=(
        1, 1), padding='same', name=name + "_Branch_3_a_max")(x)
...
def InceptionV1(include_top=True,
    ...
    input_shape = _obtain_input_shape(
        ...
        # include_top=include_top #new Keras does not have this parameter
    ...
    x = concatenated_block(
        x, ((64,), (96, 128), (16, 32), (32,)), channel_axis, 'Mixed_3b')
    x = concatenated_block(
        x, ((128,), (128, 192), (32, 96), (64,)), channel_axis, 'Mixed_3c')
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same',
                     name='MaxPool_4a_3x3')(x)
    # Now the '4' level inception units
    x = concatenated_block(
        x, ((192,), (96, 208), (16, 48), (64,)), channel_axis, 'Mixed_4b')
    x = concatenated_block(
        x, ((160,), (112, 224), (24, 64), (64,)), channel_axis, 'Mixed_4c')
    x = concatenated_block(
        x, ((128,), (128, 256), (24, 64), (64,)), channel_axis, 'Mixed_4d')
    x = concatenated_block(
        x, ((112,), (144, 288), (32, 64), (64,)), channel_axis, 'Mixed_4e')
    x = concatenated_block(
        x, ((256,), (160, 320), (32, 128), (128,)), channel_axis, 'Mixed_4f')
    x = MaxPooling2D((2, 2), strides=(2, 2), padding='same',
                     name='MaxPool_5a_2x2')(x)
    # Now the '5' level inception units
    x = concatenated_block(
        x, ((256,), (160, 320), (32, 128), (128,)), channel_axis, 'Mixed_5b')
    x = concatenated_block(
        x, ((384,), (192, 384), (48, 128), (128,)), channel_axis, 'Mixed_5c')
    ...
    if include_top:
        ...
        x = Conv2D(classes, (1, 1), strides=(1, 1),
                   padding='valid', use_bias=True, name='Logits')(x)
    ...
    if weights == 'imagenet':
        if K.image_data_format() == 'channels_first':
            if K.backend() == 'tensorflow':
                warnings.warn('You are using the TensorFlow backend, yet you '
                              ...
                              'at ~/.keras/keras.json.')
        if include_top:
            # weights_path ='/home/wli/Training/googlenetmainmodel0227HNM_boarderpatches_noise-00-0.92.hdf5'

            # The weights_path here is from the hdf5 file saved after training.
            weights_path = '/home/wli/Training/Redo/Second_train_with_hnm/no_noise_no_norm/first-hnm-train-all-hnm/no_noise_no_norm_transfer_learning_train_acc_094_val_acc_91/googlenetv1_no_noise_no_norm_0210_hnm_transfer_learn_02.10.20_09:31_original_256_patches-03-0.91.hdf5'
            # get_file(
        ...
        model.load_weights(
            '/home/wli/Training/Redo/Second_train_with_hnm/no_noise_no_norm/first-hnm-train-all-hnm/no_noise_no_norm_transfer_learning_train_acc_094_val_acc_91/googlenetv1_no_noise_no_norm_0210_hnm_transfer_learn_02.10.20_09:31_original_256_patches-03-0.91.hdf5')
        #       model.load_weights('/home/wli/Training/googlenetmainmodel0227HNM_boarderpatches_noise-00-0.92.hdf5')
        if K.backend() == 'theano':
